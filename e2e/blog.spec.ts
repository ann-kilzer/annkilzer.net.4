...
test.describe('Blog', () => {
  test('index lists posts', async ({ page }) => {
    ...
    await expect(page.getByText(/letting go/i)).toBeVisible()
  })

  test('post card links to the post', async ({ page }) => {
    await page.goto('/blog')
    await page.getByRole('link', { name: /letting go/i }).click()
    await expect(page).toHaveURL('/blog/how-i-got-to-japan-part-2')
  })
